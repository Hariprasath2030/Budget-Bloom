...
function CardInfo({ budgetList }) {
  ...
  return (
    <div>
      {budgetList?.length > 0 ?
        <div className='mt-7 grid grid-cols-1 md:grid-cols-3 lg:grid-col-3 gap-2'>
          <div className='p-7 border rounded-lg flex items-center justify-between gap-50'>
            <div>
        ...
        :
        <div className='mt-7 grid grid-cols-1 md:grid-cols-1 lg:grid-col-3 gap-5'>
          {[1, 2, 3].map((item, index) =>
          (
            <div className='h-[110px] w-full bg-slate-200 animate-bounce rounded-lg'>
            </div>
          ))}
        </div>
      }
